...
- (void)perform
{
  ...
  CGRect leftDoorRect  = CGRectMake(viewOrigin.x, viewOrigin.y, viewSize.width , viewSize.height / 2.0f);
  CGRect rightDoorRect = CGRectMake(viewOrigin.x, viewSize.height / 2.0f, viewSize.width , viewSize.height / 2.0f);
  ...
  self.doorLayerRight.anchorPoint = CGPointMake(0.5f, 1.0f);
  ...
  // Left door image
  self.doorLayerLeft.contents = (id)[CIDoorwaySegue clipImageFromLayer:[[self.sourceViewController view] layer] size:leftDoorRect.size offsetY:0.0f];
  
  // Right door image
  self.doorLayerRight.contents = (id)[CIDoorwaySegue clipImageFromLayer:[[self.sourceViewController view] layer] size:rightDoorRect.size offsetY:-leftDoorRect.size.height];
  
  // Next view image
  self.nextViewLayer.contents = (id)[CIDoorwaySegue clipImageFromLayer:[[self.destinationViewController view] layer] size:viewSize offsetY:0.0f];
...
+ (CGImageRef)clipImageFromLayer:(CALayer *)layer size:(CGSize)size offsetY:(CGFloat)offsetY
{
...
- (CAAnimation *)zoomInAnimation
{
  ...
  CABasicAnimation *zoomInAnimation = [CABasicAnimation animationWithKeyPath:@"transform.translation.y"];
  zoomInAnimation.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionEaseIn];
  zoomInAnimation.fromValue = [NSNumber numberWithFloat:0];
  zoomInAnimation.toValue = [NSNumber numberWithFloat:0];
...
- (CAAnimation *)openDoorAnimationWithRotationDegree:(CGFloat)degree
{
  ...
  zoomInAnimation.toValue = [NSNumber numberWithFloat:-2.0f*degree];
  
    animationGroup.animations = [NSArray arrayWithObjects: zoomInAnimation, nil];
  animationGroup.duration = 1.5f;
